...
auto trim(const std::string& str, TrimStrategy strategy = std::not_fn(isspace)) -> std::string;
...
auto filter(const Container& container, Predicate&& predicate) -> Container {
    Container result;
    std::copy_if(std::begin(container), std::end(container), std::back_inserter(result),
