...
auto Server::run() -> void {
    while (true) {
        ...
        if (records.empty()) {
            ...
            this->send(this->client_sock, ret_pkt->raw(), ret_pkt->raw_size(),
                       sender);
            continue;
        }

        // Header header = HeaderBuilder(pkt->header).to_response().create();
        this->registered_handler[Record::A].process(
            Query{qname, qtype, qclass});
    }
}

auto Server::receive(int sock_fd) -> std::optional<std::pair<Packet, Address>> {
    Address addr;
    ...
    return std::pair{PacketBuilder().from_binary(buf, ret).create(), addr};
}
...
auto Server::forward(const Packet& packet) -> std::optional<Packet> {
    this->send(this->forward_sock, std::move(packet.raw()), packet.raw_size(),
               this->forward_addr);
