...
auto ServerBuilder::bind(uint16_t port) -> Server {
    ...
    int on = 1;
    setsockopt(this->server.client_sock, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));

    if (::bind(this->server.client_sock, (sockaddr*)&client_sin, sizeof(client_sin)) < 0) {
    ...
    on = 1;
    setsockopt(this->server.forward_sock, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));

    if (::bind(this->server.forward_sock, (sockaddr*)&forward_sin, sizeof(forward_sin)) < 0) {
...
auto ServerBuilder::init() -> ServerBuilder& {
    ...
    int ret = inet_pton(AF_INET, this->forward_ip.data(), &this->server.forward_sin.sin_addr);

    if (ret <= 0) {
        err_quit(("Can't convert IPv4 address for " + this->forward_ip).c_str());
    }
...
auto RecordBuilder::set_type(const std::string& type) -> RecordBuilder& {
    static std::map<std::string, Record::Type> type_value = {
        {"A", Record::Type::A},      {"NS", Record::Type::NS}, {"CNAME", Record::Type::CNAME},
        {"SOA", Record::Type::SOA},  {"MX", Record::Type::MX}, {"TXT", Record::Type::TXT},
...
auto RecordBuilder::set_class(const std::string& r_class) -> RecordBuilder& {
    ...
    class_value = {{"IN", Record::Class::IN},
                   {"CS", Record::Class::CS},
                   {"CH", Record::Class::CH},
                   {"HS", Record::Class::HS}};
...
auto RecordBuilder::set_rdata(const std::vector<std::string>& data) -> RecordBuilder& {
...
auto PacketBuilder::write(void* data, size_t dlen) -> PacketBuilder& {
    std::copy_n(reinterpret_cast<uint8_t*>(data), dlen, this->buffer + this->nbytes);
    this->nbytes += dlen;
    return *this;
}
...
auto PacketBuilder::create() -> Packet {
    ...
    std::copy_n(this->buffer, header_len, reinterpret_cast<uint8_t*>(&this->packet.header));
